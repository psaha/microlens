...
integer fit
real Rp,Rn,a,b,error,chi2,parameters(5)
logical crescent, disk, gaussian
...
parameters(5) = fit
	...
	if (crescent) then
		call lightcurveCrescent(parameters,chi2) 
		write(*,*) "Crescent", chi2
	else if (disk) then
		call lightcurveDisk(Rp,error,chi2)
		write(*,*) "Uniform Disk", chi2
	else if (gaussian) then
		call lightcurveGauss(parameters,chi2)
		write(*,*) "Gaussian Disk", chi2
	end if
...
!*
	...
	pixmax,ilines,iamp,ipoints,ix,iy,i,fit
	...
	y0,x_end,y_end,x_start,y_start,x_diff,y_diff,xxx,yyy,value
      ...
      real error, chi2,sigma,Rn,a,b,parameters(5),likelihood
...
!*
!*
	open (111,file='data_crescent.txt')	!Cl data
...
!		write(*,*) i0,data_crescent1(i),value,data_crescent2(i)
		if (i0.ge.(-800.).and.i0.le.(-600.)) then
		chi2 = chi2 + ((value - data_crescent2(i+fit))/(data_crescent2(i+fit)*error))**2
!		write(*,*) i0,data_crescent1(i+fit)
		end if
            endif
	 enddo

	likelihood = exp(-chi2/2)
         do i0 = -2*ipix,2*ipix
	    xxx = x_start + i0*cosalpha
	    yyy = y_start + i0*sinalpha
	    ixxx = nint(xxx)
...
subroutine lightcurveDisk(parameters,chi2)
!*
!*      reads   unformatted  (pixel)  data of 1000^2 pixels from file IRIS401
!*       draws a line through two points, determines lightcurve
!*	for circular source, radius sigma,  with flat profile 
...
!*
	...
	ilines,iamp,ipoints,ix,iy,i
      ...
      character*80 fileread, filewrite
      parameter(ipoints = 505)
      real  x(ipoints),y(ipoints), xx(ipoints),yy(ipoints),parameters(5)
      real data_disk1(4*ipix),data_disk2(4*ipix),chi2,error,sigma
sigma = parameters(1)
error = 0.05
!*
!* name of magnification pattern to be read:
...
!*
!*
	open (111,file='data_disk.txt')	!Cl data
...
!	         write(23,1010)i0,xxx,yyy,pix(ixxx,iyyy),value,ixxx,iyyy
		...
		if (i0.ge.(-800.).and.i0.le.(-600.)) then
		chi2 = chi2 + ((value - data_disk2(i))/(data_disk2(i)*error))**2
		end if
...
subroutine lightcurveGauss(parameters,chi2)
!
!      reads   unformatted  (pixel)  data of 1000^2 pixels from file IRIS401
!       draws a line through two points, determines lightcurve
!	for circular source, radius sigma,  with flat profile 
...
!
      ...
      parameter(ipix=1000,ipix1=500)
      integer*2 pix(ipix,ipix),pix1(ipix1,ipix1),i
      ...
      character*80 fileread, filewrite
      parameter(ipoints = 505)
      real  x(ipoints),y(ipoints), xx(ipoints),yy(ipoints),sigma
      real data_gauss1(4*ipix),data_gauss2(4*ipix),chi2,error,parameters(5)
...
!
	...
	open (111,file='data_gauss.txt')	!Cl data
...
!
		...
		if (i0.ge.(-800.).and.i0.le.(-600.)) then
		chi2 = chi2 + ((value - data_gauss2(i))/(data_gauss2(i)*error))**2
		end if
!
!
!
