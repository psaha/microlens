...
\author[Tomozeiu et al]{Mihai Tomozeiu$^{1,2}$
...
\subsection{Lightcurve of the crescent shaped source}\label{subsec:lcurvcres}
...
\caption{\label{fig:M87_plots} The one dimensional profile obtained
  from the numerical integration along the ordinate axis of a
...
error as well.  To do so, we first generate a microlensing lightcurve
...
\caption{\label{fig:cc_forward} Mock lightcurve fitted with the
  ...
  for $r_{1/2}/v_p$ (solid histogram) and $R_n/R_p$ (dashed
  histogram).  Vertical dashes mark the correct values.  Inferred are
  $r_{1/2}/v_p=0.90^{+0.15}_{-.07}$ and $R_n/R_p=0.32^{+0.19}_{-0.08}$
  (90\% confidence).}
\end{figure}
...
\caption{\label{fig:cc_backward} Like Figure~\ref{fig:cc_forward},
  ...
  fit. Inferred are $r_{1/2}/v_p=0.92^{+0.06}_{-0.04}$ and
  $R_n/R_p=0.32^{+0.12}_{-0.07}$ (90\% confidence).}
\end{figure}
...
\caption{\label{fig:gc_forward} Like Figures~\ref{fig:cc_forward} and
  ...
  Inferred is $r_{1/2}/v_p=0.91^{+0.05}_{-0.01}$ (90\% confidence).}
\end{figure}
...
level.  It is, of course, important that the observations have
...
$R_n/R_p$ are still recovered, though less well; the correct
value of $r_{1/2}/v_p$ lies outside 90\% of the MCMC distribution,
but within 99\%.   The parameters $a/R_p$ and $b/R_p$
(not shown in the Figure) giving the orientation of the crescent were
not correctly recovered; but this expected, because the AB and CB
tracks have different orientations with respect to the caustics
crossed.  The slight degradation of the fit can be understood as
follows.  The source track will not, in general, be normal to the
caustic, but will instead have a non-normal velocity component.  If
the caustic is perfectly straight, the velocity component along the
caustic makes no difference.  (Hence our model only considers only the
normal velocity component $v_p$).  But in fact the caustics, although
clean folds do have some curvature.  The combination of a curving
caustic and non-normal velocity of the source makes the orientation of
the source twist with respect to the caustic.  This twist would be
different for crossings along the AB and CB tracks.
...
Namely, the shape of the caustic boundary in the proximity of the source in the respective plane can be approximated by
 a line due to reason that the local radius of curvature of the caustic is orders of magnitude greater than the half-light
...
Nevertheless, the respective displacement is relevant for the calculation
...
The observer would basically detect only the lightcurve of such a source.
 As described in section~\ref{subsec:lcurvcres} the timing of the onset and 
...
In the case of a high-quality lightcurve, provided the caustic is a
fairly clean fold, or equivalently, the source is much smaller than
the local radius of curvature of the caustic, the crossing times of 
the bright and dark areas (cf.~Figure~\ref{fig:char_points}) could
be estimated by simply identifying signatures on the
...
statistics.  In Section~\ref{sec:numerics}, we take a
...
which is perpendicular to the caustic.

% The respective velocity is an
