...
Due to the nature of the caustic and the monotonic behaviour of the magnification map on both sides of the caustic. , the previously mentioned characteristic points are inherited by the microlensing lightcurve. The points on the temporal dimension $t_1, t_2, t_3$ and $t_4$ corresnpond to instances in time when the fold is aligned with $p_1, p_2, p_3$ and $p_4$, respectively. For a constant relative velocity $v_p$ between the source and the caustic there is a simple relation between the points $p_i$ and instances $t_i$:
...
\begin{figure}
\centering
	\includegraphics[width = 0.48\textwidth]{plots/infinite_fold_ellipsess.eps}
\caption{\label{fig:ellipse_profile} Elliptical sources with constant and gaussian distributed brightness.}
\end{figure}

\subsection{Extension to ellipticaly shaped crescent sources}

We further consider a source built from the superposition of two unalligned ellipses of constant surface brightness. Analogous to the previously presented crescent source the dark ellipse is interior to the bright ellipse and the distance between their centers projected on the lines perpendicular and parallel to the fold are $a$ and $b$. The semidimensions are ($a_p, b_p$) for thebright ellipse and ($a_n, b_n$) for the dark ellipse. The semimajor axis of the ellipses for the angles $\delta_p$ and $\delta_n$ with the caustic resulting in projected widths $2 h_p$ and $2h_n$ of the two ellipses on the axis perpendicular to the caustic. 

The two dimensional description of the source is:

\begin{equation}
S_{2D}^{CEC} = S_{2D}^{CEP} - S_{2D}^{CEN}
\end{equation}

Where

\begin {equation}
S_{2D}^{CEP} = \frac{S_0^{CEC}}{2 \pi \left(a_p b_p - a_n b_n \right)} \Theta \left( 1 - \frac{\left( \left( p -p_{sp} \right) \sin{\delta_p} + \left( q - q_{sp} \right) \cos{\delta_p} \right)^2}{a_p^2}  - \frac{\left( \left( p -p_{sp} \right) \cos{\delta_p} + \left( q - q_{sp} \right) \sin{\delta_p} \right)^2}{ b_p^2} \right), 
\end {equation}
and
\begin {equation}
S_{2D}^{CEN} = \frac{S_0^{CEC}}{2 \pi \left(a_p b_p - a_n b_n \right)} \Theta \left( 1 - \frac{\left( \left( p -p_{sn} \right) \sin{\delta_n} + \left( q - q_{sn} \right) \cos{\delta_n} \right)^2}{a_n^2}  - \frac{\left( \left( p -p_{sn} \right) \cos{\delta_n} + \left( q - q_{sn} \right) \sin{\delta_n} \right)^2}{ b_n^2} \right), 
\end {equation}.

Analogous to the previous three equations the following relations can be obtained:

\begin{equation}
        h_p = \frac{v_p \left( t_4 -t_1 \right)}{2}, 
\end{equation}

\begin{equation}
        h_n = \frac{v_p \left( t_3 -t_2 \right)}{2}, 
\end{equation}
...
The time instances used in the previous equations are analogous to the ones considered in the crescent source formed from the superposition of constant surface brightness disks.

Moreover we consider a source described by an elliptical gaussian brightness function shadowed by a dark ellipse. In this case the two dimensional description of the source is given by:

\begin {equation}
S_{2D}^{GEC} = \frac{S_0^{GEC}}{2 \pi a_p b_p} \exp \left( - \frac{\left( \left( p -p_{sp} \right) \sin{\delta_p} + \left( q - q_{sp} \right) \cos{\delta_p} \right)^2}{2 a_p^2}  - \frac{\left( \left( p -p_{sp} \right) \cos{\delta_p} + \left( q - q_{sp} \right) \sin{\delta_p} \right)^2}{2 b_p^2} \right) \Pi \left(p,q\right)
\end {equation}

where the function
\begin{equation}
\Pi \left(p,q\right) = 1 - \Theta \left( 1 - \frac{\left( \left( p -p_{sn} \right) \sin{\delta_n} + \left( q - q_{sn} \right) \cos{\delta_n} \right)^2}{2 a_n^2}  - \frac{\left( \left( p -p_{sn} \right) \cos{\delta_n} + \left( q - q_{sn} \right) \sin{\delta_n} \right)^2}{2 b_n^2} \right). 
\end{equation}

\begin{figure}
\centering
	\includegraphics[width = 0.48\textwidth]{plots/ellipse_profile.eps}
        \includegraphics[width = 0.48\textwidth]{plots/ellipse_lightcurve.eps}
\caption{\label{fig:ellipse_profile} The one dimensional profile and resulting lightcurves of the elliptical sources.}
\end{figure}
